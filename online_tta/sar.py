...
class SAR(nn.Module):
    ...
    def forward(
        ...
        for _ in range(self.steps):
            loss, ema, reset_flag, outputs, text_embed, text_feat = forward_and_adapt_sar(
                text_input,
                image_feats_norm,
...
def forward_and_adapt_sar(
    ...
    text_embed2 = model.get_text_embeds(text_input.input_ids, text_input.attention_mask)
    text_feat2 = model.get_text_feat(text_embed2)
    text_feats_norm2 = F.normalize(text_feat2, dim=-1)
    sims2 = image_feats_norm @ text_feats_norm2.t()
    outputs2 = sims2.t()
    ...
    # print(loss)
    return loss, ema, reset_flag, outputs, text_embed, text_feat
